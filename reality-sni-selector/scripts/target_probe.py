...
def dig_cname(hostname: str, timeout: float = 4.0) -> tuple[list[str], str]:
    ...
    if not dig:
        return [], "missing"
    try:
        proc = subprocess.run([dig, "+time=2", "+tries=1", "+short", "CNAME", hostname], capture_output=True, text=True,
...
def tls_probe_ip(hostname: str, ip: str, timeout: float = 5.0) -> dict[str, Any]:
    ...
    try:
        ...
        return {"success": False, "ip": ip, "elapsed_ms": round((time.perf_counter() - start) * 1000.0, 3),
...
def http_head_ip(hostname: str, ip: str, timeout: float = 6.0, max_header_bytes: int = 32768) -> dict[str, Any]:
    ...
    try:
        with socket.create_connection((ip, 443), timeout=timeout) as raw:
            with context.wrap_socket(raw, server_hostname=hostname) as sock:
                request = f"HEAD / HTTP/1.1\r\nHost: {hostname}\r\nUser-Agent: reality-sni-selector/3\r\nConnection: close\r\n\r\n".encode("ascii")
        ...
        for line in lines[1:]:
            if ":" in line:
                ...
                key = key.strip().lower()
                if key in {"server", "location", "via", "x-cache", "x-served-by", "x-vercel-id", "x-amz-cf-id", "cf-ray", "x-cdn"}:
        ...
        return {"success": False, "status": None, "headers": {}, "elapsed_ms": round((time.perf_counter() - start) * 1000.0, 3), "error": type(exc).__name__}


def classify_front_door(hostname: str, cnames: list[str], cname_backend: str, head: dict[str, Any]) -> dict[str, Any]:
    evidence: list[str] = []
    provider = None
    for cname in cnames:
        for name, pattern in CDN_CNAME_PATTERNS:
    ...
    if not provider and ("cloudflare" in server or "cf-ray" in headers):
        provider, evidence = "Cloudflare", ["header:cloudflare"]
    if not provider and ("vercel" in server or "x-vercel-id" in headers):
        provider, evidence = "Vercel", ["header:vercel"]
    if not provider and "x-amz-cf-id" in headers:
        provider, evidence = "CloudFront", ["header:x-amz-cf-id"]
    if not provider and ("fastly" in server or "fastly" in headers.get("via", "").lower()):
        provider, evidence = "Fastly", ["header:fastly"]
    if provider:
        return {"class": "PUBLIC_CDN", "provider": provider, "evidence": evidence}

    if cname_backend == "missing":
        return {"class": "UNKNOWN_TOOLING", "provider": None, "evidence": ["cname_backend_missing"]}
    if cname_backend == "error":
        return {"class": "UNKNOWN_EDGE_EVIDENCE", "provider": None, "evidence": ["cname_query_error"]}
    if not cnames:
        return {"class": "DIRECT_LIKELY", "provider": None, "evidence": ["no_cname_observed"]}
    if all(same_site(hostname, cname) for cname in cnames):
        return {"class": "DIRECT_LIKELY", "provider": None, "evidence": ["same_site_cname"]}
    return {"class": "UNKNOWN_EDGE_EVIDENCE", "provider": None, "evidence": ["external_unknown_cname"]}


def gate_candidate(candidate: dict[str, Any], *, dns_observations: int = 2, tls_samples_per_ip: int = 2, timeout: float = 5.0) -> dict[str, Any]:
    ...
    if not ips:
        ...
        return {**candidate, "dns": dns, "tls": [], "http": None, "front_door": {"class": "UNKNOWN_TOOLING", "evidence": []},
                "hard_rejections": hard, "review": review, "warnings": warnings, "eligibility": "HARD_REJECTED"}
    if dns.get("volatile"):
    ...
    if successes:
        for ip in ips:
            ...
            head["ip"] = ip
            heads.append(head)
            front_rows.append(classify_front_door(hostname, cnames, cname_backend, head))
            if head.get("status") == 429:
                break
    else:
        heads.append({"success": False, "headers": {}, "status": None, "ip": ips[0]})
        front_rows.append(classify_front_door(hostname, cnames, cname_backend, heads[0]))

    public = next((r for r in front_rows if r.get("class") == "PUBLIC_CDN"), None)
    if public:
        front = dict(public)
    elif any(r.get("class") == "UNKNOWN_EDGE_EVIDENCE" for r in front_rows):
        front = {"class": "UNKNOWN_EDGE_EVIDENCE", "provider": None, "evidence": sorted({e for r in front_rows for e in r.get("evidence", [])})}
    elif any(r.get("class") == "UNKNOWN_TOOLING" for r in front_rows):
        front = {"class": "UNKNOWN_TOOLING", "provider": None, "evidence": sorted({e for r in front_rows for e in r.get("evidence", [])})}
    else:
        front = {"class": "DIRECT_LIKELY", "provider": None, "evidence": sorted({e for r in front_rows for e in r.get("evidence", [])})}
    front["cnames"] = cnames
    front["cname_backend"] = cname_backend
    if front["class"] == "PUBLIC_CDN":
    ...
    return {**candidate, "dns": dns, "current_ipv4": ips, "tls": tls_rows, "http": heads, "front_door": front,
            "hard_rejections": hard, "review": review, "warnings": warnings, "eligibility": eligibility}
