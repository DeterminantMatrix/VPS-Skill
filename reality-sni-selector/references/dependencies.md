...
If the exact v4.4 worker is already ready, no transfer is performed.
...
v4.4 may bootstrap or upgrade only its own fixed worker files; this is not a system-package installation. Installing missing Python/curl/sing-box/dig packages, changing sudo/SSH privileges, or repairing unrelated system state remains a separate maintenance action.
