...
After deployment, verify that invoking the wrapper without `run` returns `FIXED_COMMAND_REQUIRED`. Then, in authorized maintenance mode, invoke `ssh -T <declared-alias> /usr/local/bin/reality-sni-target-worker` without `run`. Require `FIXED_COMMAND_REQUIRED` plus worker identity showing protocol 4, implementation 4.1, and the expected six-file manifest. This must produce no candidate traffic. Start a new normal selection run afterward. If the fixed path or its interpreter cannot start, classify it as `TARGET_WORKER_UNAVAILABLE`.
